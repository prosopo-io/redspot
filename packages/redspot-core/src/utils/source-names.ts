...
export function validateSourceNameFormat(sourceName: string) {
  ...
  if (normalizeSourceName(sourceName) !== sourceName) {
    throw new RedspotError(ERRORS.SOURCE_NAMES.INVALID_SOURCE_NOT_NORMALIZED, {
      name: sourceName
    });
  }
}
...
export async function validateSourceNameExistenceAndCasing(
  ...
  if (trueCaseSourceName !== sourceName) {
    throw new RedspotError(ERRORS.SOURCE_NAMES.WRONG_CASING, {
...
export async function localPathToSourceName(
  ...
  if (normalized.startsWith('..')) {
    throw new RedspotError(ERRORS.SOURCE_NAMES.EXTERNAL_AS_LOCAL, {
      path: localFileAbsolutePath
    });
  }

  if (normalized.includes(NODE_MODULES)) {
    throw new RedspotError(ERRORS.SOURCE_NAMES.NODE_MODULES_AS_LOCAL, {
      path: localFileAbsolutePath
    });
  }
